...
void
leg_cgen_print_operand (CGEN_CPU_DESC cd,
			   int opindex,
			   void * xinfo,
			   CGEN_FIELDS *fields,
			   ...
			   int length)
{
  ...
  switch (opindex)
    ...
    case LEG_OPERAND_SRC1 :
      print_keyword (cd, info, & leg_cgen_opval_h_gr, fields->f_src1, 0);
      break;
    case LEG_OPERAND_SRC2 :
      print_keyword (cd, info, & leg_cgen_opval_h_gr, fields->f_src2, 0);
      break;
...
cgen_print_fn * const leg_cgen_print_handlers[] =
{
  print_insn_normal,
};
...
void
leg_cgen_init_dis (CGEN_CPU_DESC cd)
{
  leg_cgen_init_opcode_table (cd);
  leg_cgen_init_ibld_table (cd);
  cd->print_handlers = & leg_cgen_print_handlers[0];
  cd->print_operand = leg_cgen_print_operand;
}
		   ...
		   int length)
{
  ...
  for (syn = CGEN_SYNTAX_STRING (syntax); *syn; ++syn)
      ...
      if (CGEN_SYNTAX_CHAR_P (*syn))
	{
      ...
      /* We have an operand.  */
      leg_cgen_print_operand (cd, CGEN_SYNTAX_FIELD (*syn), info,
...
int
print_insn_leg (bfd_vma pc, disassemble_info *info)
{
...
#define CGEN_BFD_ARCH bfd_arch_leg
#endif
  arch = info->arch;
  ...
  if (! cd)
	...
	abort ();
      ...
      cd = leg_cgen_cpu_open (CGEN_CPU_OPEN_ISAS, prev_isa,
