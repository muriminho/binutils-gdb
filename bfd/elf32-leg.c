...
#define lm32_elf_hash_table(p) \
  ((is_elf_hash_table ((p)->hash)					\
    && elf_hash_table_id (elf_hash_table (p)) == LEG_ELF_DATA)		\
...
{
  elf_elfheader (abfd)->e_machine = EM_LEG;
  elf_elfheader (abfd)->e_flags &=~ EF_LEG_MACH;
  switch (bfd_get_mach (abfd))
    {
      case bfd_mach_lm32:
	elf_elfheader (abfd)->e_flags |= E_LEG_MACH;
	break;
      default:
	abort ();
    }
...
#define ELF_ARCH		bfd_arch_leg
#define ELF_TARGET_ID		LEG_ELF_DATA
#define ELF_MACHINE_CODE	EM_LEG
#define ELF_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM		leg_elf32_vec
#define TARGET_BIG_NAME		"elf32-leg"
